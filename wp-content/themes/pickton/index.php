<?php pickton_bunch_global_variable(); 
	...
	if( $wp_query->is_posts_page ) {
		...
		if(pickton_set($_GET, 'layout_style')) $layout = pickton_set($_GET, 'layout_style'); else
		$layout = pickton_set( $settings, 'archive_page_layout', 'right' );
		$sidebar = pickton_set( $settings, 'archive_page_sidebar', 'default-sidebar' );
		$bg = pickton_set($settings, 'archive_page_header_img');
		$title = pickton_set($settings, 'archive_page_header_title');
	}
	$layout = pickton_set( $_GET, 'layout' ) ? pickton_set( $_GET, 'layout' ) : $layout;
    ...
    <section class="page-title" <?php if($bg):?>style="background-image:url(<?php echo esc_url($bg)?>);"<?php endif;?>>
        <div class="auto-container">
            <h1><?php if($title) echo wp_kses_post($title); else esc_html_e('Blog', 'pickton');?></h1>
        </div>
    </section>
    <!--End Page Title-->
