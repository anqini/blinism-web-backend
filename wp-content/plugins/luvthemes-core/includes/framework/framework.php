<?php
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/class.redux_instances.php';
        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/lib.redux_instances.php';
    }
    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux CDN class
        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/class.redux_cdn.php';

        // Redux API class  :)
        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/class.redux_api.php';

        // General helper functions
        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/class.redux_helpers.php';

        // General functions
        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/class.redux_functions.php';
        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/class.p.php';

        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/class.thirdparty.fixes.php';

        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/class.redux_filesystem.php';

        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/class.redux_admin_notices.php';

        // ThemeCheck checks
        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/themecheck/class.redux_themecheck.php';

        // Welcome
        require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/welcome/welcome.php';
        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( esc_html__( 'Options panel created using %1$s', 'fevr' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'fevr' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'fevr' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'fevr' );
                }
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    if ( $this->args['dev_mode'] == true || Redux_Helpers::isLocalHost() == true ) {
                        require_once LUVTHEMES_CORE_PATH . '/includes/framework/core/dashboard.php';
                        new reduxDashboardWidget( $this );

                        if ( ! isset ( $GLOBALS['redux_notice_check'] ) ) {
                            require_once LUVTHEMES_CORE_PATH . '/includes/framework/core/newsflash.php';
            ...
            public function vc_fixes() {
                if ( redux_helpers::isFieldInUse( $this, 'ace_editor' ) ) {
                    ...
                    call_user_func('wp_'.'deregister_script', 'wpb_ace');
                }
            }
            ...
            private function _internationalization() {
                 ...
                 * @param string     'fevr'  text domain
                 */
                //                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'fevr' );
                //
                ...
                load_plugin_textdomain( 'fevr', false, $basepath . 'languages' );
            }
            ...
            private function _tracking() {
                if (false && file_exists( LUVTHEMES_CORE_PATH . '/includes/framework/inc/tracking.php' ) ) {
                    require_once LUVTHEMES_CORE_PATH . '/includes/framework/inc/tracking.php';
            ...
            public function _enqueue() {
                require_once LUVTHEMES_CORE_PATH . '/includes/framework/core/enqueue.php';
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'fevr' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'fevr' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_html__( 'moving the mouse over', 'fevr' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_html__( 'clicking', 'fevr' );
                    }

                    // Construct message
                    $msg = sprintf( esc_html__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'fevr' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_html__( 'Hints', 'fevr' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_html__( 'Default', 'fevr' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            private function _register_extensions() {
                $path    = LUVTHEMES_CORE_PATH . '/includes/framework/inc/extensions/';
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid security credential.  Please reload the page and try again.', 'fevr' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid user capability.  Please reload the page and try again.', 'fevr' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'fevr' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                ...
                if ( isset( $return_array ) ) {
                    if ( $return_array['status'] == "success" ) {
                        require_once LUVTHEMES_CORE_PATH . '/includes/framework/core/panel.php';
                        ...
                        if ( ! is_array( $field['required'][0] ) && count( $field['required'] ) == 3 ) {
                            ...
                            foreach ( $field['required'] as $required ) {
                                if ( ! is_array( $required[0] ) && count( $required ) == 3 ) {
                                    $parentValue = isset($GLOBALS[ $this->args['global_variable'] ][ $required[0] ]) ? $GLOBALS[ $this->args['global_variable'] ][ $required[0] ] : '';
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) >= 0 ) {
                                        $msg = _luv_kses(__( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'fevr' ));
                    ...
                    if ( ! empty( $this->args['share_icons'] ) ) {
                        foreach ( $this->args['share_icons'] as $idx => $arr ) {
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) >= 0 ) {
                                        $msg = _luv_kses(__( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'fevr' ));
